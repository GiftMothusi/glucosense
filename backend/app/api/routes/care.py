...
BASE_URL = "https://app.glucosense.health"
...
async def delete_emergency_contact(
    ...
    result = await db.execute(
        select(EmergencyContact).where(
            and_(EmergencyContact.id == contact_id,
...
@router.delete("/portal-links/{link_id}", status_code=status.HTTP_204_NO_CONTENT)
async def revoke_portal_link(
    ...
    result = await db.execute(
        select(CarePortalLink).where(
            and_(CarePortalLink.id == link_id,
                 CarePortalLink.user_id == current_user.id)
        )
    )
...
async def list_medications(
    ...
    result = await db.execute(
        select(Medication).where(
            and_(Medication.user_id == current_user.id,
...
async def add_supply(
    ...
    supply = SupplyItem(user_id=current_user.id, **data.model_dump())
    db.add(supply)
    await db.commit()
